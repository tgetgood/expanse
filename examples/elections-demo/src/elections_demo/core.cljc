...
(defn ring-election [data year accounting]
  (->> data
       year
       accounting
       (sort-by second)
...
(defn recursive-rings [data f [year & more]]
  (if more
    [(ring-election data year f)
     (-> (recursive-rings data f more)
         (scale (/ 5 8)))]
    (ring-election data year f)))
